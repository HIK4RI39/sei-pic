    ...
    <a-modal class="image-out-painting" v-model:visible="visible" title="AI 扩图" :footer="false" @cancel="closeModal">
        ...
        <a-flex gap="16" justify="center">
            ...
            <a-button type="primary" @click="handleUpload" :loading="uploadLoading"
                v-if="resultImageUrl">应用结果</a-button>
        </a-flex>
...
import { createPictureOutPaintingTaskUsingPost, getOutPaintingTaskUsingGet, uploadPictureByUrlUsingPost } from '@/api/pictureController'
        ...
        try {
            ...
            if (res.data.code === 0 && res.data.data) {
                ...
                if (taskResult.taskStatus === 'SUCCEEDED') {
                    ...
                    // 临时处理, 打印最常见的原因, 之后可以看文档在后端补充所有错误码消息
                    if (taskResult?.message?.includes("too small or to large")) {
                        message.error('扩图任务失败, 输入图像的尺寸过小或者过大')
                    } else {
